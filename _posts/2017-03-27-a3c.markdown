...
Let's implement the algorithm now. First we'll write a small wrapper around Open
AI Gym to give us more control about what the agent sees. We'll see in the tricks section that it's important to preprocess the images. This includes something called 'frame skipping', which is where we only show the agent every $n$th frame for some small $n$.

## Wrapper around Open AI Gym ##
Open AI Gym lets us hold multiple instances of an environment. In the main training program, we create an environment using
