...
\sum_{t=0}^{T-1} \log \pi (a_t | x_t; \theta)) = \grad_\theta \EE(1) = 0$, so it
will still have expectation zero when we muliply by a constant. Hence the
following two expectations are equal
...
the advantage function. What this boils down to is estimating the value of each
state and comparing it to the value we observe. More on this next time!
